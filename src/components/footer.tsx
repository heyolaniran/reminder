    ...
    return (
        <footer className="relative z-10 pb-12 flex flex-col items-center justify-center gap-4 text-slate-500 dark:text-slate-400">
            ...
            <div className="flex items-center gap-4 text-xs font-medium">
                <Link href={`/${locale}/privacy`} className="hover:text-slate-900 dark:hover:text-white transition-colors">{tFooter('privacy')}</Link>
                <Link href={`/${locale}/terms`} className="hover:text-slate-900 dark:hover:text-white transition-colors">{tFooter('terms')}</Link>
            </div>
        </footer>
    )
}